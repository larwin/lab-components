...
Four domains, each a public barrel (`index.ts` exports tokens + interfaces + a
`register*Domain` function only):
...
- A domain imports `framework` + other domains' **public barrels** only — never
  ...
  cross-domain orchestration belongs to `applications`, not a hidden store edge.
...
- **Inbound:** `app` (registers the domains into the scope tree),
  `applications/campaign-editor` (via barrels + tokens).
- **Outbound:** `framework/core`, `framework/services`, `platform` (http/telemetry).
...
- [../applications/README.md](../applications/README.md) — where cross-domain orchestration lives.
