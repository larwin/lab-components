...
| Surfaces & navigation wave 5: Drawer/Sheet (the Overlay engine pinned to an edge — trap/restore/blocking scope unchanged), AlertDialog (role="alertdialog", no outside/Escape dismissal by default, initial focus on the least destructive action via `data-autofocus`), Pagination as a pure machine (clamped goTo/next/prev/first/last against a live `getTotal`, `setPageSize` keeps the first visible item, `paginationRange` ellipsis math in the core), Breadcrumbs (`collapseBreadcrumbs` policy + the untouched Menu machine behind the "…" trigger), Menubar (APG pattern: the bar is Navigable(horizontal)+Dismissable and *the open panel is the bar's focusedKey* — ← → switch open menus, hover follows; panels reuse the Menu composition), Splitter on the NumericValue slider profile (4th reuse — the drag machine models items between zones, not a continuous ratio; pointer and arrows converge on `number/set`, `role="separator"` ARIA derived) | ✅ implemented + tested (Node machines), demo at `/surfaces` |
...
| Dates wave 7b — the pickers: the calendarGrid machine grows a **range mode** (first select anchors — no event; the live preview is the pure derivation `calendarRange` = ordered anchor → focus, so pointer hover and keyboard arrows extend it identically; second select commits *ordered by construction* with a `rangeChange` event + localized announcement; Escape cancels the anchor through a binding that falls through when none is pending — parent overlays keep Escape; program `setRange` sync is silent and reorders swapped bounds) + the **dateField machine** (segments day/month/year in locale order from Intl.formatToParts — dd/mm fr, mm/dd en-US, Arabic-Indic digits normalized on input and rendered via formatNumber; digit typing with auto-advance and overflow-restart, spinbutton arrows wrapping on the *real* month length, Backspace clears-then-walks-back à la PIN; `change` fires only at commit points — typing "2026" never drips 2 → 20 → 202, leaving the group commits drafts via a `commit` intent; segment ARIA derived by `dateSegmentAria`, labels via Intl.DisplayNames). Primitives: DateField (spinbutton segments, hidden ISO input), DatePicker (DateField + Calendar in an Overlay on the Select/Dismissable pattern, focus restored on close), DateRangePicker (two DateFields + range Calendar; start ≤ end enforced by swap on both paths) | ✅ implemented + tested (Node machines), demo at `/dates` |
