...
| Statics wave 6: Alert/Callout (role="alert" for warning/error, role="status" otherwise), Badge, Avatar (automatic initials fallback), Card, Separator (decorative vs semantic), Skeleton (aria-hidden — the owning surface announces loading), Spinner (role="status" + sr-only label), EmptyState — deliberately machine-less (correct ARIA + theme tokens, no over-engineering), CVA variants. Dropzone/FileUpload closes the wave: the acceptance policy (`partitionFiles`: extension/mime-wildcard/size/count, Node-tested) is pure à la `characterLimit`, the verdict announced through the shared live region (assertive on rejections); the native hidden input stays the real focusable control | ✅ implemented (+ Node-tested policy), demo at `/statics` |

| Surfaces & navigation wave 5: Drawer/Sheet (the Overlay engine pinned to an edge — trap/restore/blocking scope unchanged), AlertDialog (role="alertdialog", no outside/Escape dismissal by default, initial focus on the least destructive action via `data-autofocus`), Pagination as a pure machine (clamped goTo/next/prev/first/last against a live `getTotal`, `setPageSize` keeps the first visible item, `paginationRange` ellipsis math in the core), Breadcrumbs (`collapseBreadcrumbs` policy + the untouched Menu machine behind the "…" trigger), Menubar (APG pattern: the bar is Navigable(horizontal)+Dismissable and _the open panel is the bar's focusedKey_ — ← → switch open menus, hover follows; panels reuse the Menu composition), Splitter on the NumericValue slider profile (4th reuse — the drag machine models items between zones, not a continuous ratio; pointer and arrows converge on `number/set`, `role="separator"` ARIA derived) | ✅ implemented + tested (Node machines), demo at `/surfaces` |

| Rich text-entry wave 4: Searchable grows an opt-in standalone keymap (Escape → clear _only when non-empty_ — the binding falls through on an empty field — and Enter → `search` event) powering SearchField; TextArea (TextField recipe + auto-resize + a counter from the pure `characterLimit` policy, SR-announced once when entering the warning window); TagsInput as **two cooperating machines** (the ComboBox composition in multiple mode + a horizontal Navigable over chips with an input _sentinel node_ — Backspace-on-empty-field = `nav/previous`, exit-the-chips = `nav/next`, the row's `scrollToItem` reinterpreted as DOM focus); Rating on the unchanged NumericValue slider profile (half-star pointer geometry → the same `number/set` intent); PinInput as a dedicated pure machine (segment cursor, sanitizing paste distribution, focus moves as `dom/focus-element` effects, one-shot `complete` event, program-sourced sync never steals focus) | ✅ implemented + tested (Node machines), demo at `/inputs-advanced` |

| Dates wave 7a — the pure foundation: `core/date` with **DateValue** (`{year, month, day}` — no mutable `Date` ever enters machine state; exact civil-calendar arithmetic via Hinnant's epoch-day algorithms, month-add clamps Jan 31 → Feb 28) + Intl-derived locale services with zero shipped locale data (`firstDayOfWeek` from Intl.Locale week info with a region fallback, localized weekday/month names, `Date` used only as a transient UTC carrier into Intl.DateTimeFormat) + the dedicated **calendarGrid machine** (2D logical focus: arrows = days/weeks, Home/End = locale week edges, PageUp/Down = month, Shift = year; min/max clamp instead of blocking; disabled dates focusable-but-not-selectable per APG; focus moves return as `dom/focus-element` keyed by ISO date — keyboard-source only, pointer month-nav never steals focus; month changes announced politely; program sync fully silent). Calendar primitive: ARIA grid with roving tabindex, `aria-current="date"`, RTL flips ←/→ in the keymap, focus interpreter deferred one frame for freshly mounted cells | ✅ implemented + tested (Node machines), demo at `/dates` |

| Dates wave 7b — the pickers: the calendarGrid machine grows a **range mode** (first select anchors — no event; the live preview is the pure derivation `calendarRange` = ordered anchor → focus, so pointer hover and keyboard arrows extend it identically; second select commits _ordered by construction_ with a `rangeChange` event + localized announcement; Escape cancels the anchor through a binding that falls through when none is pending — parent overlays keep Escape; program `setRange` sync is silent and reorders swapped bounds) + the **dateField machine** (segments day/month/year in locale order from Intl.formatToParts — dd/mm fr, mm/dd en-US, Arabic-Indic digits normalized on input and rendered via formatNumber; digit typing with auto-advance and overflow-restart, spinbutton arrows wrapping on the _real_ month length, Backspace clears-then-walks-back à la PIN; `change` fires only at commit points — typing "2026" never drips 2 → 20 → 202, leaving the group commits drafts via a `commit` intent; segment ARIA derived by `dateSegmentAria`, labels via Intl.DisplayNames). Primitives: DateField (spinbutton segments, hidden ISO input), DatePicker (DateField + Calendar in an Overlay on the Select/Dismissable pattern, focus restored on close), DateRangePicker (two DateFields + range Calendar; start ≤ end enforced by swap on both paths) | ✅ implemented + tested (Node machines), demo at `/dates` |

| Time wave 8a — pure time & the generalized segment machine: `core/time` with **TimeValue** (`{hour, minute, second?}` — storage always 0-23, seconds opt-in and presence-preserving through arithmetic; wrap across midnight 23:59 + 1 min = 00:00; compare/clamp/range/step-round on the seconds axis; sortable ISO round-trip) + pure **hour-cycle conversions** (h11/h12/h23/h24 display ↔ storage, round-trip tested over all 24 hours × 4 cycles) + Intl-derived services (the locale's cycle from `resolvedOptions().hourCycle` — _never guessed_; `timeFieldParts` field structure; `dayPeriodLabels` forced h12 so 24-hour locales still yield labels; `Date` only as transient UTC carrier) + **DateTimeValue** (flat civil date-time, midnight-rolling arithmetic). The dateField machine **generalized into `core/field/segments`** — one generic segment-field machine (cursor, digit typing with auto-advance/overflow-restart, wrap-or-clamp arrows with dynamic `wrapMax`, draft/commit rule, change detection through an injected `getValue`/`isEqual` projection so clamping collisions never double-fire, textual segments via `parseChar`); dateField and timeField are two _configurations_ of it (zero new reducer for time). dayPeriod is a cyclic textual segment: arrows toggle, the localized "a"/"p" initial (latin fallback) sets it outright, `aria-valuetext` announces the label. Primitives: TimeField (locale-ordered spinbutton segments, hidden ISO input), DateTimeField (DateField + TimeField in one ARIA group — composed value null until both halves complete, no null echo), DateTimePicker (Calendar + TimeField in the same Overlay; picking a date keeps it open, first pick defaults to midnight) | ✅ implemented + tested (Node machines), demo at `/time` |

| Time wave 8b — TimePicker, ONE state, FOUR shells (the "a machine, N renders" proof): a single `TimePicker` primitive with a `variant` prop (`segments \| columns \| wheel \| dial`) — a prop and not four primitives because the props contract and behavior are strictly identical, only the projection changes. **Zero new machines**: `segments` is the 8a TimeField; `columns` and `wheel` are the untouched listbox composition [Focusable+Navigable+Selectable] per column (typeahead included; the wheel just sets `selectionFollowsFocus` so arrows rotate it); `dial` reuses NumericValue on the slider profile — pointer geometry and arrow keys converge on the same `number/set` intent (the Slider pattern, 5th NumericValue reuse: NumberField, Slider, Rating, Splitter, dial). What differs is pure Node-tested policy/geometry in `core/time/picker`: option policies (minuteStep 1/5/15, min/max bounds, `isTimeDisabled` evaluated per concrete time — an hour is disabled iff every minute candidate in it is), wheel snap + inertia as one projection (offset, velocity → settled index via v²/2a, never on a disabled hole), dial angle ↔ value (0° at 12 o'clock clockwise, snap by true circular distance so steps that don't divide 60 stay exact, two rings outer 1-12 / inner 13-00 for 24-hour locales discriminated by distance ratio, single ring + dayPeriod for h12). Demo binds all four variants to the SAME controlled TimeValue | ✅ implemented + tested (Node geometry), demo at `/time` |

| Time wave 8c — timezones, zero shipped data: `core/time/zone` with pure **ZonedDateTime** (`DateTimeValue + timeZone + offsetMinutes`) — the offset of an instant in an IANA zone is _computed_ through Intl.DateTimeFormat (`timeZoneName: "longOffset"`, formatToParts), so DST is correct by construction and follows the host's tz database; instant ↔ wall-clock conversions resolve transitions the Temporal-"compatible" way (overlap → earlier instant, gap → clock jumps forward; candidate offsets probed a day on each side of the guess — probing only at the guess misses the pre-transition offset); DST-safe arithmetic (`addZonedDays` keeps the WALL time — the transition day is 23 exact hours; `addZonedMinutes` is exact duration); zone directory from `Intl.supportedValuesOf("timeZone")` with localized names (long/short/shortOffset via formatToParts) and city extraction. **Windows ↔ IANA mapping as an OPTIONAL leaf module** `core/time/windows-zones.ts`: generated from the CLDR supplemental windowsZones table (139 Windows ids, 500 territory-aware mapZone entries) by `scripts/generate-windows-zones.ts` (provenance + regeneration documented in both files), versioned in the repo, deliberately NOT re-exported by the core barrel — consumers who don't need Windows ids never ship it. TimeZoneSelect primitive: the ComboBox **untouched** — grouping by offset is pure data (offset computed at a reference instant, items sorted offset-first, labels "UTC+02:00 · City — localized name" make city/name/offset all searchable). **Temporal API decision**: out of scope until runtime support is universal; ZonedDateTime's surface is deliberately Temporal-shaped (fields + timeZone + offset) to ease that migration | ✅ implemented + tested (Node, DST summer/winter + gap/overlap proofs), demo at `/time` |

| Interactions wave 9a — Toolbar, zero new machines: the bar is the untouched [Focusable + Navigable(horizontal, wrap)] with `scrollToItem` reinterpreted as DOM focus on real buttons (the Accordion pattern) — ONE tab stop, roving tabindex, ←/→ wrap, Home/End, ↑/↓ left to inner widgets (the Select trigger opens on them); RTL flips the _stroke_ before keymap resolution (`flipHorizontalStroke`, pure — keymaps never learn about direction). Heterogeneous content with zero new logic: buttons are native buttons (activation is native DOM, global shortcuts via the ShortcutProvider), toggle groups are the Selectable behavior alone (navigation belongs to the bar — toggles are toolbar stops), the compact select is the existing Select primitive (one added `tabIndex` integration slot, machine untouched), separators are layout. Disabled items stay focusable (`aria-disabled` — APG: no navigation hole). Overflow "…": measured widths feed the pure `partitionOverflow` policy (core/layout — priority per item, trigger width reserved, dangling separators pruned, Node-tested à la collapseBreadcrumbs); overflowed items stay MOUNTED but hidden so their machines and shortcuts survive; the menu is the untouched Menu machine over the pure `overflowMenuSections` projection (group/select values appear as checks). FloatingToolbar = the same Toolbar anchored to the live text selection by the Overlay engine (virtual anchor on the range rect — the ContextMenu pattern); a wrapper, NOT a variant prop: unlike TimePicker (same contract, different projection) floating adds a lifecycle, which is composition | ✅ implemented + tested (Node: partition, flattening, RTL, menu projection), demo at `/toolbar` |

| Interactions wave 9b — Carousel, zero new track machine: the track is [Focusable + Navigable(horizontal) + **Autoplayable**] over a collection of PAGES — logical focus via `aria-activedescendant` (pages beyond the mounted window exist for the keyboard), `wrap: true` IS the keyboard loop, RTL reuses `flipHorizontalStroke` (2nd consumer). **Autoplayable is the one new behavior** of the wave, justified as a genuinely new state domain (timed rotation): `playing` + a suspension set (hover/focus/hidden), timers as the Toast pattern — the reducer emits `schedule-advance`/`cancel-advance` pairs and the adapter owns the single `setTimeout`; the invariant "a timer runs ⟺ playing ∧ no suspension" is Node-proven (every stopping path cancels, the last resume reschedules, a tick landing on the end of a non-wrapping track auto-pauses). APG rotation rules live in the reducer, not shells: page changes announced ONLY while rotation is off; program `nav/move` (controlled sync) fully silent, while autoplay ticks (program `nav/next`) DO emit `pageChange`. Drag-snap **reuses the 8b wheel geometry unchanged**: non-loop settle IS `wheelSettle` (inertia v²/2a then snap) run horizontally; the loop variant keeps the same coast unclamped and `normalizeLoopIndex` (pure modulo — no DOM cloning) maps virtual onto real pages, `loopDelta` picking the shortest signed path so the seam animates forward. Slides-per-page is pure pagination math; long tracks stay cheap via the Fenwick virtualizer (`range(offset, viewport)` on the transform offset — 10k slides ≈ 6 mounted). Dots pagination = the untouched ToggleGroup; play/pause button mandatory and first in tab order; slides `role="group"` + `aria-roledescription="slide"` (grouped pattern — tabpanel rejected: the dots are toggle buttons, not a tablist) | ✅ implemented + tested (Node: rotation invariant, APG announce rules, settle/loop math), demo at `/carousel` |

| Interactions wave 9c — ColorPicker on a pure color core, NO color machine: `core/color` is dependency-free — exact RGB ↔ HSL ↔ HSV and sRGB ↔ OKLCH (Ottosson's OKLab matrices hard-coded, round-trips proven over a grid of the RGB cube to < 1/255, reference values asserted), CSS parsing/formatting for hex/rgb()/hsl()/oklch() in both legacy-comma and modern-space syntax (parse∘format round-trips tested), WCAG contrast (relative luminance, ratio, the four AA/AAA thresholds) and ΔEOK (Euclidean OKLab distance) for "close to <named color>" announcements. The CSS named-colors table (148 entries, CSS Color 4) is a **leaf module** `core/color/named.ts` deliberately NOT re-exported by the barrel (the windows-zones pattern) — the primitive takes a `colorName` injection and never imports it. The picker composes EXISTING machines only: the 2D saturation/brightness surface is **NumericValue ×2** — one machine per axis, the 6th and 7th reuses (NumberField, Slider, Rating, Splitter, dial, area-X, area-Y); ↑/↓ resolve against the Y machine (spinbutton profile) before ←/→/Home/End hit the X machine (slider profile), and the pointer converts its position into the same two `number/set` intents; `aria-valuetext` announces BOTH axes ("saturation 40 %, luminosité 80 %"). Hue and alpha are the untouched Slider (a `trackStyle` styling slot only), hex is the TextField with core parsing committed at blur/Enter (draft → commit, invalid input reverts), R/G/B are NumberFields, swatches are a ToggleGroup (chosen over a grid Listbox: a small single-choice strip needs no 2D collection navigation). Internal truth is **HSV + alpha** — hue survives zero saturation and saturation survives black, where an RGB round-trip would erase both. EyeDropper API is an adapter-only, feature-detected option (never in the core) | ✅ implemented + tested (Node: conversion/parse round-trips, WCAG, ΔE, named table), demo at `/color` (live WCAG checker included) |
