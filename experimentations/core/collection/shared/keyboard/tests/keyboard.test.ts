...
describe('focus navigation', () => {
  ...
  it('supports context-based usage through exported low-level helpers', () => {
    const visibleIds = [...ids];
    const focusedId = 'b';
    const disabledIds = new Set<string>(['c']);
    const isFocusable = (id: string) => !disabledIds.has(id);

    expect(getNextFocusableId(visibleIds, focusedId, isFocusable)).toBe('d');
    expect(getPrevFocusableId(visibleIds, focusedId, isFocusable)).toBe('a');
    expect(getFirstFocusableId(visibleIds, isFocusable)).toBe('a');
    expect(getLastFocusableId(visibleIds, isFocusable)).toBe('e');
  });
});
